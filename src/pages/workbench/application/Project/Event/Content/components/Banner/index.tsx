...
export default ({ id, ...props }: any) => {
  const { banner, updaterBanner, updaterPublishTime } = useModel('event', (model) => model);
  ...
  const columns: ProColumns[] = [
    {
      title: '排序',
      dataIndex: 'sort',
      render: (dom, record: any) => <div style={{ wordBreak: 'break-all' }}>{record.title}</div>,
      width: 260,
    },
    {
      title: 'banner图',
    ...
    },
    {
      title: '类型',
      dataIndex: 'type',
      valueEnum: formOptionToValueenum(linkOptions),
      valueType: 'select',
      width: 120,
    },
    {
          ...
          toolBarRender={() => [
            ...
            <Button
              ...
              onClick={async () => {
                ...
                try {
                  const data = list.map((item: any) => omitBy(item, isNull));
                  await addBannerRule({
                    project_id: id,
                    data,
                  });
